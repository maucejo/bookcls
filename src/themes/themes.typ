...
// Part
#let part = it => context if states.theme.get().contains("modern") {
  part-modern(it)
...
#let minitoc = context if states.theme.get().contains("modern") {
  minitoc-modern
} else if states.theme.get().contains("classic") {
  minitoc-classic
} else if states.theme.get().contains("fancy") {
...
// Custom box
#let custom-box(title: none, icon: "info", color: rgb(29, 144, 208), body) = context if states.theme.get().contains("modern") {
